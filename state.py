...
class State(object):
    ...
    def __init__(self, device, package, activity, description, views, screenshot_path, state_id=None, start_time=None):
    ...
    def save_view_img(self, view):
        try:
            ...
            tag = self.start_time.strftime("%Y-%m-%d_%H%M%S")
            view_img_dir = os.path.join(self.device.output_dir,'views',tag)
