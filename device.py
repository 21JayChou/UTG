...
class Device:
    def __init__(self, current_state:State = None, start_time = None) -> None:
        self.adb = ADB(self)
    ...
    def get_current_state(self):
        ...
        try:
            ...
            if xml_path is None:
                ...
                screen_description = self.get_screen_description(self.package, str(views), self.activity)
            screenshot_path = self.get_screenshot()
            self.logger.info("finish getting current device state...")
            current_state = State(self,self.package, self.activity, screen_description, views, screenshot_path, start_time=self.start_time)
    ...
    def is_same_package(self):
        if self.last_package is not None and self.package != self.last_package:
            return False
        return True